...
void main() {
  ...
  group('authRedirect — unauthenticated', () {
    ...
    test('/splash from data(null) → /auth/login', () {
      final r = authRedirect(
        auth: const AsyncValue.data(null),
        onboarded: const AsyncValue.data(false),
        currentPath: '/splash',
  ...
  group('authRedirect — authenticated, not onboarded', () {
    test('non-onboarding route → /onboarding', () {
      final r = authRedirect(
    ...
    test('/auth/verify-email → no redirect (always allowed)', () {
      final r = authRedirect(
        auth: AsyncValue.data(user),
        onboarded: const AsyncValue.data(false),
    ...
    test('/auth/login while user signed in → /onboarding', () {
      final r = authRedirect(
        auth: AsyncValue.data(user),
        onboarded: const AsyncValue.data(false),
        currentPath: '/auth/login',
