...
**Questions tranchées par le PO — 2026-05-23**

| # | Question | Décision | Conséquence d'implémentation |
|---|---|---|---|
| **Q-OPEN-A** | Algo `perWeek N` : uniforme ou picker jours ? | **V1 = uniforme.** Picker jours en V2 sur HB-02. | `ScheduleHabitRemindersUseCase` répartit N occurrences sur 7 jours avec `floor(7/N)` d'écart, en commençant le lundi de la semaine courante. V2 (picker) fera l'objet d'un ADR-019 et d'une issue HB-02. |
| **Q-OPEN-B** | Grace window : minuit strict ou buffer 30 min ? | **Minuit local strict.** Pas de tolérance. | `windowEndsAt = endOfDay(scheduleDate, tz: localTz)` soit `23:59:59.999`. Le job `MarkMissedOccurrencesUseCase` tourne à 00:00:05 local pour laisser 5 s de marge système — pas un buffer utilisateur. |
| **Q-OPEN-C** | Tap `dismiss` + validation manuelle le lendemain : `late` ou `missed` ? | **`late` tant que `now < windowEndsAt + 24h`.** | `CompleteOccurrenceUseCase` : si `now ≤ windowEndsAt + 24h` → statut `late` même si l'occurrence est déjà en état `missed` local. L'utilisateur perd les points de streak mais gagne les points de complétion (cf. §4.3 — hiérarchie des statuts). Au-delà de +24h, la validation est bloquée (état `archived`). |
...
**Accepted** — 2026-05-23.
**Questions PO tranchées** — 2026-05-23 (Q-OPEN-A, Q-OPEN-B, Q-OPEN-C résolues — cf. §10).
